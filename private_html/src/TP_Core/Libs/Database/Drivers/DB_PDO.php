...
if(ABSPATH) {
    class DB_PDO extends DB_Abstract_Driver{
        ...
        public function close() {
            $this->pvt_connection = null;
        }
        public function isConnected() {
            return $this->pvt_connection instanceof PDO;
        }
        public function _query($query, array $parameters = []) {
            try {
                $result = null;
                if($this->pvt_connection instanceof PDO){
                    $result = $this->pvt_connection->prepare($query);
                }
        ...
        public function getLastId(){
            if($this->pvt_connection instanceof PDO){
                return (int)$this->pvt_connection->lastInsertId();
            }
            return null;
        }
        public function quote($value) {
            if($this->pvt_connection instanceof PDO){
                return (int)$this->pvt_connection->quote($value);
            }
            return null;
        }
        public function getServerVersion() {
            if($this->pvt_connection instanceof PDO){
                return $this->pvt_connection->getAttribute(PDO::ATTR_SERVER_VERSION);
            }
            return null;
        }
        protected function _connect($host, $user, $pass, $db, array $options = []) {
            if ($this->pvt_connection === null) {
                ...
                try {
                    $this->ptt_pdo = $this->pvt_connection = new PDO("mysql:host=".$host.";dbname=".$db.";port=".$options['port'].";charset=".$options['charset'], $user, $pass,
                        [
